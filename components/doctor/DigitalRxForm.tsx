...
/* ==================== INSERT API (NEW) ==================== */
export type InsertTarget = "chiefComplaints" | "doctorNote";

export type DigitalRxFormHandle = {
  /**
   * Insert text into a target field.
   * - mode: "append" (default) or "replace"
   * - ensureBullets: prefix every non-empty line with "• "
   * - dedupe: remove duplicate bullet lines (trimmed, case-sensitive)
   */
  insert: (
    target: InsertTarget,
    text: string,
    options?: {
      mode?: "append" | "replace";
      ensureBullets?: boolean;
      dedupe?: boolean;
      prependLabel?: string; // e.g., "AI:"
    }
  ) => void;
};

/* --------------------- small helpers (NEW) --------------------- */
function toBullets(s: string): string {
  const lines = s
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter(Boolean)
...
function dedupeBullets(s: string): string {
  const lines = s
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter(Boolean);
  ...
  for (const l of lines) {
    const key = l.replace(/^\u2022\s*/, "• ").trim(); // normalize bullet
    if (!seen.has(key)) {
      seen.add(key);
      out.push(key);
    }
...
const DigitalRxForm = forwardRef<DigitalRxFormHandle, DigitalRxFormProps>(
  ({ value, onChange, onSave, onSubmit, autoBMI = true }, ref) => {
    /* --------------------- Derived + effects --------------------- */
    ...
    const [openVitalsMore, setOpenVitalsMore] = useState(false);
    ...
    /* -------------- Imperative Insert API (NEW) -------------- */
    useImperativeHandle(ref, () => ({
      insert: (target, text, options) => {
        const mode = options?.mode ?? "append";
        const ensureBullets = options?.ensureBullets ?? true;
        const dedupe = options?.dedupe ?? true;
        const prefix = options?.prependLabel?.trim();

        let payload = text || "";
        if (ensureBullets) payload = toBullets(payload);
        if (prefix) payload = `${prefix} ${payload}`;

        if (target === "chiefComplaints") {
          const current = value.clinical.chiefComplaints || "";
          const next =
            mode === "replace" ? payload : mergeAppend(current, payload);
          patch("clinical", {
            chiefComplaints: dedupe ? dedupeBullets(next) : next,
          });
        } else if (target === "doctorNote") {
          const current = value.plan.doctorNote || "";
          const next =
            mode === "replace" ? payload : mergeAppend(current, payload);
          patch("plan", { doctorNote: dedupe ? dedupeBullets(next) : next });
          // If you prefer to target Advice instead, switch to:
          // patch("plan", { advice: dedupe ? dedupeBullets(next) : next });
        }
      },
    }));
                    ...
                    <MiniCard title="Women Health">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        ...
                        <LabeledCombo
                          ...
                          value={
                            value.vitals.womensHealth?.cycleLengthDays ?? ""
                          }
...
export function CardShell({
  ...
  return (
    <section
      className={borderless ? "bg-transparent shadow-none p-0" : "bg-white shadow-md p-3"}
    >
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        <div className="shrink-0 flex items-center gap-2">
          {actionsRight ? (
            <div className="flex items-center gap-1.5">{actionsRight}</div>
          ) : null}
          {moreLabel ? (
            <button
              type="button"
...
/* --------------- Prescription Table (as in your file) --------------- */
