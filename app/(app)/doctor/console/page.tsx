...
/* ------------------------------------------------------------ */
/* 🔐  Extract logged-in user session + branch + role from cookies */
/* ------------------------------------------------------------ */
...
const sessionRaw = getCookie("aran.session");
const session = sessionRaw ? decodeBase64Json(sessionRaw) : null;

// Extract needed values
const userId = session?.id || "";
const roleFromSession = session?.legacyRole?.toLowerCase() || "";
const activeBranch =
  getCookie("aran.activeBranch") || session?.legacyBranches?.[0] || "";
...
function getFormComponent(dept: string) {
  switch (dept) {
    case "gen":
      return require("@/components/doctor/console/digitalrx/genForm").default;
...
export default function DoctorConsolePage() {
  ...
  /* ------------------------------------------------------------ */
  /* 🔥 1. READ SESSION COOKIE → PARSE JWT                        */
  /* ------------------------------------------------------------ */
  ...
  type AranSession = {
    ...
    clinicId: string;
    status: string;
    access: Array<{ branchId: string; role: string }>;
    legacyRole: string;
    legacyBranches: string[];
  };

  function readSession(): AranSession | null {
    if (typeof document === "undefined") return null;
    const raw = document.cookie
      .split("; ")
      .find((x) => x.startsWith("aran.session="));
    if (!raw) return null;

    const token = raw.split("=")[1];
    try {
      return jwtDecode(token) as AranSession;
    } catch {
      return null;
    }
  }
  ...
  async function loadStaffJson() {
    ...
    if (!res.ok) throw new Error("staff.json not found");
  ...
  /* ------------------------------------------------------------ */
  /* 4. Stable dependency values → ALWAYS strings                 */
  /*    prevents hook-order mismatch                              */
  /* ------------------------------------------------------------ */
  ...
  /* ------------------------------------------------------------ */
  /* 5. Load department based on (userId + role + branch)         */
  /* ------------------------------------------------------------ */
  useEffect(() => {
    async function loadDept() {
      try {
        ...
        // Find the exact staff record for the logged-in user
        const me = staff.find(
          (s) =>
            s.id === depUser &&
        ...
        console.error("Error loading staff dept:", e);
        setDoctorDept("default");
      }
    }
