...
// ----------------------------------------------------------------------------
// Purpose
//  - This is the doctor's primary workspace to create ABDM health records:
...
//  - Add your actual form wiring / API calls later (save→FHIR→submit)
//
// UX Notes
//  - Left: sticky tab bar for record types
//  - Right: active record editor in a card
//  - Top: patient bar with ABHA indicator
...
type RecordKey = "wellness" | "opconsult" | "prescription" | "immunization" | "lab";
...
export default function DoctorConsultationsPage() {
  // ---- Patient context (replace with actual patient picker/state) ----
  const [patientName, setPatientName] = useState("");
  const [patientAbha, setPatientAbha] = useState<string | null>(null); // e.g., "12-3456-7890-12"
  ...
  const [meds, setMeds] = useState<Med[]>([
    { id: uid(), name: "", route: "PO", dose: "", frequency: "", duration: "", instructions: "" },
  ]);
  ...
  const validate = useCallback(() => {
    ...
    if (active === "opconsult" && !opChiefComplaints.trim()) {
      setToast({ type: "error", message: "Chief complaints are required for OPConsult." });
      return false;
    }
    if (active === "prescription" && meds.some((m) => !m.name.trim())) {
      setToast({ type: "error", message: "Please enter at least one medication name." });
      return false;
    }
    if (active === "immunization" && !immVaccine.trim()) {
      setToast({ type: "error", message: "Vaccine name is required for Immunization record." });
      return false;
    }
    if (active === "lab" && !labTestName.trim()) {
      setToast({ type: "error", message: "Test name is required for Lab record." });
      return false;
    }
    if (active === "wellness" && (!wlHeight || !wlWeight)) {
      setToast({ type: "error", message: "Height and Weight are needed to compute BMI." });
      return false;
    }
    return true;
  }, [active, patientName, opChiefComplaints, meds, immVaccine, labTestName, wlHeight, wlWeight]);
  ...
  const handleValidate = useCallback(() => {
    ...
    setToast({ type: "success", message: `Validation passed for ${labelFor(active)}.` });
  }, [validate, active]);

  const handleSubmit = useCallback(() => {
    if (!validate()) return;
    // Business: submit FHIR to your API; on success, optionally share (SMS/ABDM link)
    setToast({ type: "success", message: `${labelFor(active)} submitted successfully.` });
  }, [validate, active]);
  ...
  const copyOpToPrescription = useCallback(() => {
    ...
    setMeds((curr) => {
      const text = [opDiagnosis && `Dx: ${opDiagnosis}`, opPlan && `Plan: ${opPlan}`, opAdvice && `Advice: ${opAdvice}`]
        .filter(Boolean)
        .join(" | ");
      return curr.map((m, i) => (i === 0 && !m.instructions ? { ...m, instructions: text } : m));
    });
    setToast({ type: "info", message: "OPConsult notes copied to prescription instructions." });
    ...
    <div className="space-y-4">
      ...
      <div className="ui-card p-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl" style={{ background: "var(--secondary)" }} />
            <div>
              <div className="text-sm font-semibold">Consultations</div>
              <div className="text-[11px] text-gray-500">Create ABDM health records for the selected patient</div>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
            {/* Patient picker (replace with your real picker) */}
            <div className="flex-1 sm:flex-none">
              <label className="block text-[11px] font-medium text-gray-600 mb-1">Patient</label>
              <input
                className="ui-input"
            ...
            <div className="sm:w-64">
              <label className="block text-[11px] font-medium text-gray-600 mb-1">ABHA</label>
              <div className="flex items-center gap-2">
                <input
                  className="ui-input"
                  placeholder="xx-xxxx-xxxx-xx"
                  value={patientAbha ?? ""}
                  onChange={(e) => setPatientAbha(e.target.value || null)}
                />
                <span className={["ui-chip", patientAbha ? "text-[--on-secondary]" : ""].join(" ")}
                  style={patientAbha ? { background: "var(--secondary)", color: "var(--on-secondary)", borderColor: "var(--secondary)" } : {}}
                >
                  {patientAbha ? "Linked" : "Unlinked"}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)] gap-4">
        ...
        <aside className="ui-card p-3 h-max sticky top-24">
          <div className="text-xs font-semibold text-gray-600 mb-2">Record Types</div>
        ...
        <section className="space-y-4">
          ...
          <div className="ui-card p-4">
            <div className="grid md:grid-cols-4 gap-3 text-sm">
              ...
              <Field label="Consultation Date">
                <input type="date" className="ui-input" defaultValue={today()} />
              </Field>
              <Field label="Time">
                <input type="time" className="ui-input" defaultValue={nowTime()} />
              </Field>
              <Field label="Care Context (auto)">
                <input className="ui-input" placeholder="Generated on Submit" disabled />
              </Field>
            </div>
          </div>
          ...
          {active === "opconsult" && (
            <div className="ui-card p-4">
              <HeaderRow title="OPConsult Record" subtitle="Findings, Diagnoses and Plan" />
              <div className="grid md:grid-cols-2 gap-3">
                <Field label="Chief Complaints*">
                  <Textarea value={opChiefComplaints} onChange={(e) => setOpChiefComplaints(e.target.value)} placeholder="e.g., Fever, cough for 3 days" />
                </Field>
                <Field label="Examination">
                  <Textarea value={opExamination} onChange={(e) => setOpExamination(e.target.value)} placeholder="Vitals, systemic exams" />
                </Field>
                <Field label="Diagnosis">
                  <Textarea value={opDiagnosis} onChange={(e) => setOpDiagnosis(e.target.value)} placeholder="Primary/secondary Dx" />
                </Field>
                <Field label="Plan">
                  <Textarea value={opPlan} onChange={(e) => setOpPlan(e.target.value)} placeholder="Investigations, follow‑up, lifestyle" />
                </Field>
                <Field className="md:col-span-2" label="Advice / Notes">
                  <Textarea value={opAdvice} onChange={(e) => setOpAdvice(e.target.value)} placeholder="Any additional instructions" />
                </Field>
              </div>
          ...
          {active === "prescription" && (
            <div className="ui-card p-4">
              <HeaderRow title="Prescription" subtitle="Add medications and instructions" />
              <div className="space-y-3">
                {meds.map((m, idx) => (
                  <div key={m.id} className="grid md:grid-cols-6 gap-3 items-start">
                    <Field label={`Medicine ${idx + 1}*`}>
                      <input className="ui-input" value={m.name} onChange={(e) => updateMed(m.id, { name: e.target.value })} placeholder="e.g., Paracetamol 500mg" />
                    </Field>
                    <Field label="Route">
                      <select className="ui-input" value={m.route} onChange={(e) => updateMed(m.id, { route: e.target.value as Med["route"] })}>
                        <option>PO</option><option>IV</option><option>IM</option><option>Topical</option><option>Inhale</option><option>SC</option>
                      </select>
                    </Field>
                    <Field label="Dose">
                      <input className="ui-input" value={m.dose} onChange={(e) => updateMed(m.id, { dose: e.target.value })} placeholder="e.g., 500 mg" />
                    </Field>
                    <Field label="Frequency">
                      <input className="ui-input" value={m.frequency} onChange={(e) => updateMed(m.id, { frequency: e.target.value })} placeholder="e.g., 1‑0‑1" />
                    </Field>
                    <Field label="Duration">
                      <input className="ui-input" value={m.duration} onChange={(e) => updateMed(m.id, { duration: e.target.value })} placeholder="e.g., 5 days" />
                    </Field>
                    ...
                    <Field className="md:col-span-6" label="Instructions">
                      <Textarea value={m.instructions} onChange={(e) => updateMed(m.id, { instructions: e.target.value })} placeholder="e.g., After food, increase fluids" />
                    </Field>
                  </div>
                ))}
                <div>
                  <button className="btn-primary" onClick={() => addMed()}>+ Add Medicine</button>
                </div>
              </div>
            </div>
          )}
          ...
          {active === "immunization" && (
            <div className="ui-card p-4">
              <HeaderRow title="Immunization Record" subtitle="Capture vaccine details" />
          ...
          {active === "lab" && (
            <div className="ui-card p-4">
              <HeaderRow title="Lab Record" subtitle="Add new lab test or results" />
              <div className="grid md:grid-cols-3 gap-3">
                <Field label="Test Name*"><input className="ui-input" value={labTestName} onChange={(e) => setLabTestName(e.target.value)} placeholder="e.g., CBC" /></Field>
                <Field label="Sample Date"><input type="date" className="ui-input" value={labSampleDate} onChange={(e) => setLabSampleDate(e.target.value)} /></Field>
                <Field label="Result"><input className="ui-input" value={labResult} onChange={(e) => setLabResult(e.target.value)} placeholder="e.g., Within normal limits" /></Field>
              </div>
          ...
          {active === "wellness" && (
            <div className="ui-card p-4">
              <HeaderRow title="Wellness Record" subtitle="Vitals and lifestyle assessment" />
              <div className="grid md:grid-cols-4 gap-3">
                <Field label="BP Systolic (mmHg)"><input className="ui-input" value={wlBpSys} onChange={(e) => setWlBpSys(e.target.value)} /></Field>
                <Field label="BP Diastolic (mmHg)"><input className="ui-input" value={wlBpDia} onChange={(e) => setWlBpDia(e.target.value)} /></Field>
                <Field label="Pulse (bpm)"><input className="ui-input" value={wlPulse} onChange={(e) => setWlPulse(e.target.value)} /></Field>
                <Field label="Temp (°C)"><input className="ui-input" value={wlTemp} onChange={(e) => setWlTemp(e.target.value)} /></Field>
                <Field label="SpO₂ (%)"><input className="ui-input" value={wlSpo2} onChange={(e) => setWlSpo2(e.target.value)} /></Field>
                <Field label="Height (cm)"><input className="ui-input" value={wlHeight} onChange={(e) => setWlHeight(e.target.value)} /></Field>
                <Field label="Weight (kg)"><input className="ui-input" value={wlWeight} onChange={(e) => setWlWeight(e.target.value)} /></Field>
                <Field label="BMI (auto)"><input className="ui-input" value={bmi} readOnly placeholder="Auto" /></Field>
                <Field className="md:col-span-4" label="Lifestyle Notes"><Textarea value={wlLifestyle} onChange={(e) => setWlLifestyle(e.target.value)} placeholder="Diet, exercise, sleep" /></Field>
                <Field label="Follow‑up"><input type="date" className="ui-input" value={wlFollowUp} onChange={(e) => setWlFollowUp(e.target.value)} /></Field>
              </div>
            </div>
          )}
        </section>
  ...
  function addMed() {
    setMeds((curr) => [...curr, { id: uid(), name: "", route: "PO", dose: "", frequency: "", duration: "", instructions: "" }]);
  }
}
